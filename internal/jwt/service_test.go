...
func TestService_GetUserByRefreshToken(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			userRowVals:  []interface{}{user.ID, user.Email, user.Role, user.UidNumber, user.FullName, user.StudentID, user.CreatedAt, user.UpdatedAt},
			expired:      false,
			expectErr:    false,
			setupMock: func() {
				...
				userRow := &mockRow{vals: []interface{}{user.ID, user.Email, user.Role, user.UidNumber, user.FullName, user.StudentID, user.CreatedAt, user.UpdatedAt}, err: nil}
				mockDB.On("QueryRow", mock.Anything, "-- name: GetUserByRefreshToken :one\nSELECT u.id, u.email, u.role, u.uid_number, u.full_name, u.student_id, u.created_at, u.updated_at FROM refresh_tokens r JOIN users u ON r.user_id = u.id WHERE r.id = $1\n", id).Return(userRow).Once()
			},
		},
		{
